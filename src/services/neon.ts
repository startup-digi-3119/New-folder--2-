...
export const neon = {
    ...
    from: (table: string) => ({
        ...
        update: (values: any) => ({
            ...
            match: async (queryObj: any) => {
                const keys = Object.keys(values);
                const setClause = keys.map((k, i) => `"${k}" = $${i + 1}`).join(', ');
                const matchKeys = Object.keys(queryObj);
                const whereClause = matchKeys.map((k, i) => `"${k}" = $${keys.length + i + 1}`).join(' AND ');
                const sqlStr = `UPDATE "${table}" SET ${setClause} WHERE ${whereClause} RETURNING *`;
                try {
                    const result = await query(sqlStr, [...Object.values(values), ...Object.values(queryObj)]);
                    return { data: result.rows, error: null };
                } catch (error: any) {
                    return { data: null, error };
                }
        ...
        delete: () => ({
            ...
            match: async (queryObj: any) => {
                const matchKeys = Object.keys(queryObj);
                const whereClause = matchKeys.map((k, i) => `"${k}" = $${i + 1}`).join(' AND ');
                const sqlStr = `DELETE FROM "${table}" WHERE ${whereClause} RETURNING *`;
                try {
                    const result = await query(sqlStr, Object.values(queryObj));
                    return { data: result.rows, error: null };
                } catch (error: any) {
                    return { data: null, error };
                }
